...
data = read.table("/home/calcul/Carlos/Results/Recurrence/simp/TTum330_alphaG1120/TTum330_alphaG1120.csv", sep = ',', header = TRUE)
for(i in 1:76){
  if(data$bio_rec[i] == 0){
    data$bio_rec[i] = "N"
  }
...
model = train(bio_rec ~ tum_vol + T2w_ave + ADC_ave + total_dose,
...
model = train(bio_rec ~ tum_vol + T2w_contrast_mean + T2w_diff_var_mean,
              data, method = "rf", trControl = myControl)
roc = (roc(predictor = model$pred$Y, response = model$pred$obs))
auc(predictor = model$pred$Y, response = model$pred$obs)
plot.roc(roc, add = TRUE, col = "orange", lty = 1, main = "ROC AUC")

model = train(bio_rec ~ X8w_int_tum_area_norm,
              data, method = "glm", trControl = myControl)
roc = (roc(predictor = model$pred$Y, response = model$pred$obs))
auc(predictor = model$pred$Y, response = model$pred$obs)

    ...
    for i = 1:nCombPar
        ...
        for j = 1:P
            temp1 = load([pathTissue, char(fileNames(selOut1)), '_'...
                num2str(i - 1), '_', sim, num2str(j - 1), '.res']);
...
pAllTissues = pAllDoses ./ initVascDens;

%%
% This block plots, for a given combination of parameters (dose value), the
...
for i = 1:nTissues
    pathTissue = [path, 'Tissue', num2str(i)];
    temp = load([pathTissue, char(fileNames(selOut1)),'_0.res']);
    output1(i) = temp(2, outputCol(selOut1));
    temp = load([pathTissue, char(fileNames(selOut2)),'_0.res']);
...
for nTissue = 1:nTissues
    pathTissue = [path, 'Tissue', num2str(nTissue)];
    %     par = load([pathTissue, '/combPar.res']);
    %     nCombPar = size(par, 1);
    par = 2;
    nCombPar = 1;
...
p1 = polyfit(initDens(:, 2)', meanP(:, 1)', deg);
...
for i = 1:nCombPar
    ...
    for nTissue = 1:nTissues
        ...
        plot(output1(2:end, 2), output2(2:end, 2))
        plot(output1(2:end, 2), (p1(1) * initDens(nTissue, 2) + p1(2)) *...
            output1(2:end, 2) + p2(1) * initDens(nTissue, 2) + p2(2))
        hold off
        grid on
        xlabel(char(outputNames(selOut1)))
        ylabel(char(outputNames(selOut2)))
    ...
    for nTissue = 1:nTissues
        ...
        output2 = cell2mat(meanOutput2(nTissue, i));
        plot(output2(2:end, 1), output2(2:end, 2))
        plot(output2(2:end, 1), (p1(1) * initDens(nTissue, 2) + p1(2)) *...
            output1(2:end, 2) + p2(1) * initDens(nTissue, 2) + p2(2))
        hold off
        grid on
        %         ylim([0, inf])

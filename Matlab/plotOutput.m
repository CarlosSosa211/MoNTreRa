...
output = load([path, '/morris', char(fileNames(selOut)), '_', num2str(n), '.res']);
...
colormap(jet)
for i = 1 : nPar
    scatter(output(i,1), output(i,2), 200, color(i), 'filled', shape(mod(i, length(shape)) + 1))
end
...
xlabel('\mu*')
ylabel('\sigma')
title(['Tissue ', num2str(n), ' - ', char(outputNames(selOut))], 'fontsize', 20)
